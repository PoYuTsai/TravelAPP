...
> 日期：2026-01-22
> 狀態：✅ 已實作完成
> 進度更新：見 `docs/updates/2026-01-22-structured-editor-and-quality.md`
...
- [x] 新增航班欄位（arrivalFlight, departureFlight）
- [x] 新增服務選項欄位（guideService, childSeat, extraVehicle）
- [x] 新增車輛欄位（vehicleCount, vehicleType）
- [x] 新增跨欄位驗證規則
- [x] 測試 schema 變更不影響現有資料

### Phase 2：基本資訊表單 ✅

- [x] 建立 StructuredBasicInfoForm 元件
- [x] 實作航班下拉選單（含「其他」選項 + 智慧提示）
- [x] 實作人數欄位（成人、小朋友、年齡、身高）
- [x] 實作服務選項勾選框（導遊、座椅、雙條車 + 數量 + 天數）
- [x] 實作車輛安排（台數、車型、行李備註）
- [ ] 實作「重置為範本」按鈕（未實作，非必要）

### Phase 3：報價表格 ✅

- [x] 建立 StructuredQuotationTable 元件
- [x] 實作每日包車費用表格（日期自動產生）
- [x] 實作其他費用區塊（導遊、座椅、雙條車、保險、外地住宿）
- [x] 實作自動計算邏輯
- [x] 實作數值連動（基本資訊 → 報價）
- [ ] 實作「新增自訂項目」功能（未實作，可後續擴充）
- [x] 實作驗證提示

### Phase 4：行程內容面板 ✅

- [ ] 實作可收合的範本參考面板（未實作，使用說明提示代替）
- [x] 實作原生 textarea 編輯區（避免游標跳掉）
- [ ] 實作 Tab 鍵支援（未實作）
- [ ] 實作快捷插入按鈕（未實作）
- [ ] 實作「重置為範本」按鈕（未實作）
...
- [x] 整合所有區塊到 syncFromTextAction
- [x] 測試「同步更新」寫入 Sanity
- [x] 測試驗證規則阻擋儲存
- [x] 測試匯出功能（LINE/PDF/Excel）不受影響
- [x] 測試現有資料可正常開啟編輯
...
- [x] 可選擇常用航班或自訂輸入
- [x] 服務選項（導遊/座椅/雙條車）可設定數量和天數
- [x] 報價自動計算正確（含外地住宿補貼）
- [x] 驗證連動正常（勾選服務 ↔ 報價項目）
- [x] 行程編輯無游標跳掉問題
- [x] 匯出功能（LINE/PDF/Excel）正常運作
- [x] 現有資料可正常開啟編輯
