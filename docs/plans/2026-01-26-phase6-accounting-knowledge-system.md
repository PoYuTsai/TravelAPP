...
| 分類 | 筆數 | 內容 |
|------|------|------|
| 完整範本 | 3 | 報價確認、收到訂金、行前提醒 |
| 服務說明 | 5 | 包含/不包含、用車時間、導遊、小費 |
| 行前提醒 | 4 | 入境須知、匯率、TDAC、天氣 |
| 付款相關 | 2 | 訂金付款、後續流程 |
| 連結 | 1 | 官網連結 |
| 其他 | 1 | 感謝語 |
...
**新增功能：**
1. 建立 `scripts/update-notion-knowledge-base.mjs` 批次更新腳本
2. 新建飯店資料庫（NOTION_HOTEL_DB）

**資料新增：**
| 資料庫 | 新增筆數 | 內容 |
|--------|---------|------|
| 門票 | +8 | 大象活動 3 筆、清萊景點 5 筆 |
| 餐廳 | +8 | 咖啡廳 8 筆（Ristr8to、Akha Ama 等）|
...
- [x] Notion API Integration Token（用於建立知識庫）
...
**Commit:** `7ca46ec` refactor: unify brand positioning to 清邁親子包車
...
**變更重點：**
- 統一「自由行」→「包車」（品類更精準）
- 加入 Eric & Min 名字（人格化信任）
- IG/FB Bio 同步更新

**修改檔案：**
| 檔案 | 變更內容 |
|------|----------|
| `layout.tsx` | 標題、描述、OG 圖片 alt |
| `Hero.tsx` | 預設標題、圖片 alt |
| `Footer.tsx` | 品牌描述 |
| `manifest.ts` | PWA 描述 |
| `page.tsx` | 首頁預設標題 |
| `ToursPageClient.tsx` | 行程頁標題 |
| `landingPage.ts` | Sanity 預設值 |
