...
- **刀4 ✅ 完成** — commit `588c21e`(本體) + `79d58ad`(review fix)。`oa-contact-recorder.ts`＝閘 `AI_AGENT_OA_CAPTURE_ENABLED`(off)＋fail-safe try-swallow，處理 `oa_follow`(冪等不覆寫 followedAt)／`oa_text`(append messages、cap 20、firstMessageAt 首設、保留 sheetWritten)，非 line_oa／空 userId／空白文字／其他 kind 全早退。two-stage review：spec ✅（只動兩新檔、無偷改刀1–3、無 over-build）、code quality ✅（修 1 Important＝log 從自創鬆散型別對齊專案 `AgentLogger`＋用已註冊事件碼 `store_write_failed`{reason:'oa_contact_record_failed'}，刀5 可直接傳既有 logger；順帶閉合「log throw 冒泡」Minor；補 2 測釘死 follow 分支保留 messages/firstMessageAt/sheetWritten）。13 passed，tsc 對動到檔零錯。**Minor #4（gate 解析全 codebase ~10 處重複，抽共用 helper）刻意不做＝跨檔重構超出本刀，列後續。** base SHA = `d1f1d1f`。
- **刀5 ✅ 完成** — commit `1b88986`（本體，review 零 finding 不需 fix commit）。在 `defaultEventHandler` archiver seam 之後插 `await recordOaContactEvent(event, store, { env: process.env, log })`（`env`/`log` 傳法與同檔 archiver 一致）。**暗坑驗掉**：`oa_follow` 端到端從 route→normalizer→handler 真的到得了 seam 並落 store——刀4 的 normalizer 分支（follow→oa_follow，line_oa，非 null）讓它進得了 `defaultEventHandler`，route.ts:113-117 只在 normalizer 回 null 時 `continue`，故 follow 不被早段擋。註記：`oa_follow` 也會續流進 routeCommand→`handleCreateOrUpdateCase`（刀4 normalizer 既有副作用＝bare follow 也建 OA case，非刀5 scope，列觀察）。TDD 4 測（gate-on follow/text 落 store、gate-off byte-identical＋建案路徑不受影響 via `getByLineUserId`、partner-group 短路不寫 OA namespace；test 1/2 端到端鎖 wiring）。two-stage review（fresh agent）：**CLEAN 零 finding**——接線位置無早退跳過、byte-identical 成立（recorder 第一行查閘）、測試非假綠。line-agent 全綠 1899，eslint 兩改動檔乾淨，tsc 對動到檔零錯（既有測試檔類型錯與本刀無關）。base SHA = `e49b9ae`。
- **刀6 ✅ 完成** — commit `7a6939d`（本體，review CLEAN 零 finding 不需 fix commit）。`sheets-client.ts`＝不引 googleapis，手刻 JWT RS256（Web Crypto `RSASSA-PKCS1-v1_5`/SHA-256）換 access token 打 Sheets REST `values:append`；`signAssertion` 做成注入 seam，主流程測試零 crypto 零網路。TDD 3 測：token 交換＋append URL 三要素（range encode／`:append`／`valueInputOption=USER_ENTERED`）＋`Bearer` header＋`{values}` body 端到端斷言、append 失敗拋**非 minified**錯（`/403.*PERMISSION_DENIED/`）、**smoke 不注入 signAssertion 真走 `defaultSignAssertion`**（真 `generateKey`→export pkcs8→組 PEM→驗三段非空 JWT，預設簽章路徑非假綠）。fresh-agent review：**CLEAN**（正確性/錯誤處理/安全/測試品質全過；2 非阻擋 suggestion＝token 未快取、assertion 未 encode，皆非現行 bug 且超本刀 scope，不動）。ads 目錄回歸 16 passed（sheets-client 3＋oa-contact-recorder 13）無退化。base SHA = `1b88986`。
- **刀7 ✅ 完成** — commit `4c3b5d3`（本體）+ `960fcfe`（review fix）。`summary-adapter.ts`＝`summarizeOaInquiry` 把 OA 訊息摘要成 {inquiry, headcount, amount}。閘 `AI_AGENT_ADS_SUMMARY_ENABLED`(off)。**fail-open 永不 throw**：閘關／日 cap 非 'ok'／LLM 拋／回非 JSON／缺 inquiry 一律退原文節錄（`EXCERPT_MAX=60`＋「…」）。cap gate 在呼叫 LLM 前擋，對齊 `anthropic-call.ts` BUDGET GATE；`recordSpend` 與真 LLM(`callAnthropicMessages` Haiku) 綁定留給刀8 composition root。TDD 13 測。fresh-agent review 抓 **1 Important＝真 bug**：`excerpt()` 原在 `try` 外預算 fallback，`messages` undefined（`OaContactRecord.messages` 型別 optional）或 `m.text` 非字串會冒泡破鐵律→`960fcfe` 補防禦（`Array.isArray` 守＋`(m?.text ?? '').trim()`）＋2 fail-open 測釘死。回歸 ads 29 passed，tsc 對動到檔零錯。base SHA = `7a6939d`。⚠️ 刀8 接線提醒：本層 `AdsCostCap.checkBudget(): 'ok'|string` 與真 `DailyCostCap.checkBudget(): CostCapCheck`(帶 `.outcome`) 型別不同，composition root 要綁 `outcome==='ok'?'ok':outcome` 轉接，別直塞。
- **刀8 ✅ 完成** — commit `ed45a10`（本體，fresh-agent review **CLEAN 零 must-fix**，不需 fix commit）。三檔＋一改：`run-daily-sheet.ts`(可測 runner)、`route.ts`(GET `/api/cron/ads-daily-sheet` 薄殼 composition root)、`vercel.json`(`0 2 * * *` UTC = 09:00 Asia/Bangkok)、`daily-cost-cap.ts` 把 `bangkokDay` 改 **export**(DRY，不自寫第二份時區換算)。**runner**：掃 `listOaContactRecords()` 篩 `firstMessageAt && !sheetWritten`→summarize→`appendRows`→標 `sheetWritten`(**append 成功後才標**)；失敗進 catch 只 `failed++`＋log，**不標記(下次 cron 重試)、不擋同批他筆**。TDD 4 測(append+標記／冪等 re-run／follow-only 無 firstMessageAt 跳過／單筆炸不擋他筆且不標記)。**composition root 接線**：`getStore`(`line/webhook-runtime`)、`createKvClientFromEnv`(`storage/kv-store`)、`callAnthropicMessages`(`observability/anthropic-call`)、`createDailyCostCap`、`createSheetsClient`，全復用既有不自造。**cap 型別轉接**(刀7 提醒已落實)：包 `summaryCostCap` 讓 `checkBudget`回 `.outcome==='ok'?'ok':outcome`；`recordSpend` 為 `AdsCostCap` 介面所迫的**死碼**(summarize 從不呼叫，實際記帳只在 `callAnthropicMessages` 內用原始 dailyCap 一次，**無雙扣**——review 專查已釐清)。route auth：空 `CRON_SECRET` 或 `Authorization` 不符 `Bearer <secret>` 一律 401；全程 try-catch 錯誤不 minified；`export const dynamic = 'force-dynamic'`。fresh-agent review 七項全 CLEAN(雙扣/冪等/auth/bangkokExport/import 真實性/時區/scaling)，唯一非阻擋觀察＝runner 每日全量掃描、已寫過者永留掃描集，量大再加封存游標。回歸 `src/lib/line-agent` **150 files / 1919 tests 全綠**，四交付檔 tsc 0 error。base SHA = `960fcfe`。**⏳ 剩唯一步＝Step 8 真連線 smoke**(唯一觸網)：需 Eric 一次性前置＝分享目標 Sheet `1DhSGSmaPtlszo4k-nc0wjrHVj1oaaWusk962dYrhjNU` 編輯權給 service account email＋設 env(`AI_AGENT_GOOGLE_SA_JSON` base64／`AI_AGENT_ADS_SHEET_ID`／`AI_AGENT_ADS_SHEET_RANGE`／`CRON_SECRET`／`AI_AGENT_OA_CAPTURE_ENABLED`／`AI_AGENT_ADS_SUMMARY_ENABLED`／`ANTHROPIC_API_KEY`)，再 `curl -H "Authorization: Bearer $CRON_SECRET" https://<preview>/api/cron/ads-daily-sheet` 驗一列＋回 `{ok:true,appended:N}`，重打驗冪等(appended=0)。
- **✅ 刀1–8 全實作完成**。整條管線(被動記錄→Haiku 摘要→每日 cron→Sheets append)碼與測就緒，僅餘 Eric 一次性 SA 授權＋env 的真連線 smoke。
- **刀9 ✅ 完成** — commit `7dd3b9f`。每日轉換表加「客人姓名」欄，best-effort LINE displayName（取不到退代稱／userId），TDD。連帶把線上 Google Sheet 的 Table 物件從舊 4 欄升成乾淨 9 欄（對齊 buildRow 的 9 欄，程式↔表已對齊；歷史分頁 202603-06 刻意不動）。⚠️ Sheet 是「Table 物件」非普通格線：values API 只塞得動文字、改不了欄型別/範圍，改 Table 結構須用 batchUpdate `updateTable`（scratchpad/fix-table.mjs 範本，SA JWT 換 token 流程同 `sheets-client.ts`）。
- **✅ 刀1–9 整批 merge 上 main（2026-07-07）** — merge commit `0f6c8aa`。決策：整批 merge（外科式已排除——ads 生於 `src/lib/line-agent/ads/` 且硬 import agent 的 `line/webhook-runtime`／`storage/*`／`observability/*`，無法乾淨抽離；分支領先 main 434 commit）。**merge 前 `next build` gate 揪出真 bug**：`sheets-client.ts:41` SA JWT `crypto.subtle.sign` 型錯——TS 5.7 lib.dom 收緊 `BufferSource` 成 ArrayBuffer-backed，`TextEncoder().encode()` 型別 `Uint8Array<ArrayBufferLike>`（含 SharedArrayBuffer）被拒；**unit test(ts-jest) 放過、只有 next build tsc 抓得到**。修法 commit `3cac101`：`utf8` 回傳收斂 `Uint8Array<ArrayBuffer>`（runtime 永遠真 ArrayBuffer，純型別過度保守）。教訓：`npm run build 2>&1 | tail` 的 exit code 是 tail 的 0、會騙人，改落檔＋明確 `$?`。README 自動合乾淨（兩側 Phase 7 編輯相同、build-trigger 純新增），main 的 `scripts/google-ads/daily-export.js`（Google Ads Script，與刀8 cron 互補不撞）保留。**Vercel Production 部署 Ready（`0f6c8aa`、Error Rate 0%）＝刀1-9 全套碼已上正式站**。runtime 由 gating 壓住：三閘 default off、`AI_AGENT_ADS_SUMMARY_ENABLED=false`、prod 無 `ANTHROPIC_API_KEY`（走刀7 節錄 fallback）。**⏳ 剩 Eric 後台：①目標 Sheet `1DhSGSmaPtlszo4k-nc0wjrHVj1oaaWusk962dYrhjNU` 編輯權授予 service account email（否則 append 403）②確認 6 env 真值 ③等 UTC 02:00 首跑或手動 `curl -H "Authorization: Bearer $CRON_SECRET" https://chiangway-travel.com/api/cron/ads-daily-sheet` 驗 `{ok:true,appended:N}`＋重打驗冪等(appended=0)**。
