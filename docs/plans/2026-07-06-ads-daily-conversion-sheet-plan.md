...
- **刀7 ⏳ 待做（下一步從這開始）** — Haiku 摘要 adapter（閘＋日 cap，敗→原文節錄，見 Task 7）。範本：LLM 封裝 `observability/anthropic-call.ts`、日 cap `observability/daily-cost-cap.ts`、閘慣例 archiver。base SHA = `7a6939d`。
