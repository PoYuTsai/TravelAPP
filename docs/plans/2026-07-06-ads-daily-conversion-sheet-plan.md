...
- **刀5 ⏳ 待做（下一步從這開始）** — 接線進 webhook-runtime seam（`webhook-runtime.ts:298-304` archiver 段之後插 `recordOaContactEvent`，gate-off byte-identical）。**暗坑**：`oa_follow` 新 kind 可能進不了 `defaultEventHandler`（webhook route 早段擋非建案類），接線時端到端追 `route.ts:113-117` 確認 follow 送得進 recorder seam。log 現已是 `AgentLogger`，接線時把 webhook 既有 logger 直接傳入。base SHA = `79d58ad`。
- 刀6–8 待做（見上各 Task；刀6 Sheets client、刀7 Haiku 摘要 adapter 與 1–5 無依賴，但逐刀 subagent 不並行派以免同分支 commit 衝突）。
