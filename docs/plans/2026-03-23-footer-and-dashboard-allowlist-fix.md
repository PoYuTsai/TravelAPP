...
This maintenance slice removes the public-facing Claude credit from the site footer and guarantees Eric's primary Gmail address can access the protected dashboard and accounting routes on the production branch.
...
- `npx vitest run --exclude ".worktrees/**" src/lib/__tests__/api-auth.test.ts src/components/__tests__/Footer.test.tsx` -> passed
- `npm run lint` -> passed
- `npm run build` -> passed

## Notes

- The first targeted `npm run test:run` attempt also picked up duplicate test files inside `.worktrees/`, so verification was rerun with `.worktrees/**` excluded to isolate the production branch.

## Commit

- Feature commit: `d9158ab` `fix: remove footer credit and allow eric dashboard access`
- Docs commit: pending
