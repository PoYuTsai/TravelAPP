...
- （技術債）排行程與 partner-rag 各自建一份 TTL 快取 RagIndex（同語料兩份）；兩者觸發路徑不同且皆 gated，未合併。日後可把 partner-rag 已快取 index 共用給排行程 source。
...
7. ⏳ **【persona 航班規則收斂 RAG】**（依賴 Task 8）Task 7 的 `CI851/BR257` 仍硬寫 persona；待 Task 8 航班時刻表入 RAG，改為「依航班時刻表知識庫的早班」避免兩處真相漂移。`deriveCaseProfile` 的 knownFlight 已只認客人自報航班、不臆造，未引入第三處真相。
