...
## 落在 wiring 刀（本計畫**不**碰，列此備忘）

- composition root 把 `selectItineraryReference` 包成 `itineraryReferenceSource` 注入 responder（同 `knowledgeSource` 處）。
- case profile 推導器（從鬆散對話抽 mobility/stayArea/knownFlight/days）。
- `AI_AGENT_NOTION_RAG_ENABLED` 開閘 + 真群 preview 驗收。
- （技術債）抽共用 `callAnthropicMessages`（responder 與其他 LLM 呼叫點）。
...
1. **【I-1 範本去污 + 兩路徑共用 sanitizer】** `chiang-mai-family-5d4n-classic.md` body 目前含具體航班碼（`長榮 BR257/BR258`）與 `**markdown 粗體**`。前者正是 case 路徑 `RESIDUAL_PII_RES` 會 drop 的 token —— 同字串在真案例會被殺、卻從範本逐字注入，直接抵銷 Task 7「不臆造航班、標待確認」；後者誘導 LLM 產出 gate 會擋的 `**`/`#` 格式，拉高 tripwire degrade 率。**修法**：先把範本 body 的航班碼/`**` 清掉，再讓 `templateSkeleton()` 也過 `sanitizeItinerarySnippet`（清乾淨後不會 fail-closed），使「凡注入為骨架者皆過同一 assert」成為統一不變量。最乾淨做法＝把清過的範本 inline 成 TS 常數（順帶解 I-2）。
2. **【I-2 範本 runtime 可達性】** `itinerary-reference-source.ts` 用 `readFileSync(path.join(process.cwd(), TEMPLATE_REL))`。Vercel lambda 的 cwd／檔案 tracing **不保證**含 `docs/**`，檔案缺席即 throw。responder 的 fail-open 會吞掉（不崩，續走無骨架），但後果隱蔽：**無命中時（語料早期最常見）安全網靜默消失、LLM 從零亂編**——正是範本要防的「從零亂編」。且 throw 與 Notion 失敗共用同一 log code，無從區分。**修法**：把範本 inline 成 TS 常數（無檔案相依，首選），或 `next.config` `outputFileTracingIncludes` 把範本納進 bundle。
3. **【M-1 案例/範本來源訊號】** `selectItineraryReference` 回傳 `{source:'case'|'template'}`，但 responder 目前型別 `=> Promise<string|null>` 丟掉了 `source`。wiring 時加一個固定碼欄位（如 `referenceSource?: 'case'|'template'`）到 log，才看得出某筆 draft 用真案例或退範本 → 調語料涵蓋率的關鍵訊號。
4. **【M-2 合併雙重檢索】** `itineraryReferenceSource` 與 `caseProfileSource` 吃同一 `need`、各打一次；wiring 時併成單一 source 回 `{skeleton, profile}` 避免同一 turn 對 Notion/retrieval 打兩次（design 已提此選項）。
5. **【M-3 範本須保持無敬稱】** 若 I-1 讓範本過 sanitizer，未來範本若加 example 含「李先生」等敬稱會被整筆 drop → fallback 靜默失效。在範本檔註記「須維持無敬稱/無航班碼」。
6. **【Task 4 型別一致性】** `itineraryReferenceSource`／`caseProfileSource` 目前為 inline function type；wiring 落 `itinerary-reference-source.ts` 真 source 模組時，抽 `ItineraryReferenceSource` 型別別名收斂兩處 inline 簽名。並補一測斷言 source 收到的 `need === input.text`，把選取合約釘死。
7. **【persona 航班規則收斂 RAG】** Task 7 的 `CI851/BR257` 硬寫在 persona；Task 8 航班時刻表入 RAG 後，改為「依航班時刻表知識庫的早班」避免兩處真相漂移。
