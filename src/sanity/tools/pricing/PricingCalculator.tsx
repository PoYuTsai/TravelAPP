  ...
  <div class="pdf-container">
    ...
    <div class="section">
      ...
      <div class="price-summary">
        ...
        <div class="price-row category">
          <span>🚗 團費（${c.carCount}台車${c.guided ? '，含中文導遊' : ''}）</span>
          <span>${fmt(c.transportPrice)} 泰銖</span>
        </div>
        ${(c.perPerson?.trip?.items ?? []).map((item: any) => `<div class="price-detail">• ${item.label} ${item.quantity} 位 × ${fmt(item.unitPriceThb)}</div>`).join('')}
        ${(c.perPerson?.transferFee ?? 0) > 0 ? `<div class="price-detail">• 接送機 ${c.perPerson.transferTrips} 趟（按車計）</div>` : ''}
    ...
    <div class="includes">
      <div class="box yes">
        ...
        <ul>
          ...
          <li>• 全程包車（${c.carCount}台，含車資油費過路停車）</li>
          ${c.guided ? `<li>• 專業中文導遊</li>` : ''}
    ...
    <div class="payment-phases">
      ...
      <div class="payment-phase">
        ...
        <div class="items">
          • 團費（含車${c.guided ? '＋導遊' : ''}${c.needLuggageCar ? '＋機場行李車' : ''}）${c.childSeatCost > 0 ? '、兒童座椅' : ''}<br />
          • 統一結算超時費（若有）
        </div>
      ...
      ` : ''}
      <div class="payment-phase">
        <div class="label">📍 ${hasMealsOrTicketsOrInsurance ? '第三' : '第二'}階段｜車${c.guided ? '導' : '輛'}費（含超時結算）</div>
        <div class="timing">⏰ 送機前一天</div>
        <div class="items">
          • 團費（含車${c.guided ? '＋導遊' : ''}${c.needLuggageCar ? '＋機場行李車' : ''}）${c.childSeatCost > 0 ? '、兒童座椅' : ''}<br />
          • 統一結算超時費（若有）
      ...
      <div class="payment-phase">
        <div class="label">📍 第二階段｜車${c.guided ? '導' : '輛'}訂金 30%</div>
        <div class="timing">⏰ 同時支付</div>
        <div class="items">• 確認行程細節後支付訂金，即完成預約</div>
        <div class="amount">💰 ${fmt(Math.round(carAmount * 0.3))} 泰銖 <span style="font-weight:normal;color:#666;">≈ NT$ ${fmt(Math.round(carAmount * 0.3 / exchangeRate))}</span></div>
      </div>
      <div class="payment-phase">
        <div class="label">📍 第三階段｜車${c.guided ? '導' : '輛'}尾款 70%（含超時結算）</div>
        <div class="timing">⏰ 送機前一天</div>
        <div class="items">
          • 團費（含車${c.guided ? '＋導遊' : ''}${c.needLuggageCar ? '＋機場行李車' : ''}）${c.childSeatCost > 0 ? '、兒童座椅' : ''}<br />
          • 統一結算超時費（若有）
...
function downloadSimpleExternalQuote(
  ...
  const externalQuote = buildExternalQuoteBreakdown({
    ...
    transferFee: c.perPerson?.transferFee ?? 0,
    transferTrips: c.perPerson?.transferTrips ?? 0,
    includeAccommodation,
    includeMeals,
    includeGuide: c.guided ?? includeGuide,
  ...
  const travelerSummary = `${c.adults} 位大人${c.children > 0 ? ` + ${c.children} 位小孩` : ''}${c.infants > 0 ? ` + ${c.infants} 位嬰兒` : ''}`
...
interface SavedQuote {
  ...
  data: {
    ...
    /** Legacy saved-quote compatibility. */
    luggageCar?: boolean
    childSeatCount?: number
...
export function PricingCalculator({ variant = 'legacy' }: PricingCalculatorProps) {
  ...
  // Form states - 成人/小孩/嬰兒三段計費（12+ 全價、3-11 八折、0-2 半價）
  // 預設 6大2小＝8 佔位
  const [adults, setAdults] = useState(6)
  const [children, setChildren] = useState(2)
  const [infants, setInfants] = useState(0)
  const people = adults + children + infants  // 總佔位人數（嬰兒也佔位，用於配車、保險）
    ...
    if (parsed.days.length > 0) {
      ...
      const newCarFees = parsed.days.map((day, index) => {
        ...
        if (isFirstDay && day.title?.includes('抵達')) {
          ...
          defaultPrice = 700
          defaultCost = 500
        } else if (day.title?.includes('金三角') || day.title?.includes('美塞') || day.rawText?.includes('金三角') || day.rawText?.includes('美塞')) {
          type = 'goldentriangle'
          defaultPrice = 6600
          defaultCost = 5200
  ...
  const loadQuote = (quote: SavedQuote) => {
    ...
    if (quote.data.adults !== undefined) {
      ...
      setInfants(quote.data.infants || 0)
    } else {
      // 舊格式：假設全部是成人
    ...
    // 舊 quote 存的 0.93 是方向錯誤的 legacy 匯率，載入時重設為統一 fallback
    if (quote.data.exchangeRate !== undefined) {
      setExchangeRate(quote.data.exchangeRate < 0.9 ? DEFAULT_THB_PER_TWD : quote.data.exchangeRate)
    }
  ...
  useEffect(() => {
    const passengerCarCount = people <= 3 ? 1 : Math.ceil(people / 9)
    const requiredLuggageCars = people >= 4
      ? countLuggageCheckCars(people, passengerCarCount)
      : 0
    setLuggageCarCount(requiredLuggageCars)
  }, [people])
  ...
  const calculation = useMemo(() => {
    ...
    const requiredLuggageCars = people >= 4
      ? countLuggageCheckCars(people, passengerCarCount)
      : 0
    const safeLuggageCarCount = requiredLuggageCars
    const perPerson = buildPerPersonQuote({
      ...
      luggageVansPerAirportDay: safeLuggageCarCount,
    })
    ...
    // 行李：先分車；6–7 不加、8–9 加一台、10–14 不加、15–18 加一台。
    ...
    const needLuggageCar = safeLuggageCarCount > 0
    ...
    // Meal（可選擇不含餐費；嬰兒不計餐）
    const mealCost = includeMeals ? (adults + children) * mealLevel * mealDays : 0

    // Car cost 側：黃表每日成本 × 台數（內部毛利參考，售價改由人頭團費決定）
    let carCostTotal = 0, legacyCarPriceReference = 0
    dailyCarFees.forEach((d: any) => {
      carCostTotal += (d.cost || 0) * carCount
      legacyCarPriceReference += (d.price || 0) * carCount
    })

    // Guide cost 側：只採 engine 的實際總成本，兩台 Van 不得用舊 1,500/日低估。
    const guideCost = guided
      ? perPerson.trip?.guideCostThb ?? (perPerson.manualQuoteRequired ? null : 0)
      : 0
    const guidePrice = 0

    // 售價側：人頭團費已含 guide sell anchor；manual quote 必須維持 null。
    const carPriceTotal = perPerson.groupTourPrice

    // Luggage：只有人工確認的行李車會攤入團費，不另重複列項。
    const luggageCost = 0

    // Child Seats：新報價只提供 0–2 歲嬰兒座椅；舊報價欄位仍保留相容。
    const childSeatCost =
      totalChildSeatCount * config.childSeatPerDay * childSeatDays

    const transportCost = guideCost === null ? null : carCostTotal + guideCost
    const transportPrice = carPriceTotal === null
      ? null
      : carPriceTotal + guidePrice + luggageCost + childSeatCost
    const transportProfit = transportPrice === null || transportCost === null
      ? null
      : transportPrice - transportCost - luggageCost - childSeatCost
    ...
    // Totals
    const totalCost = transportCost === null
      ? null
      : accommodationCost + mealCost + transportCost + effectiveTicketCost + effectiveThaiDressCost + insuranceCost + luggageCost + outboundStayCost
    const totalPrice = transportPrice === null
      ? null
      : accommodationCost + mealCost + transportPrice + effectiveTicketPrice + effectiveThaiDressPrice + insuranceCost + outboundStayCost

    const yourTotalProfit = transportProfit === null
      ? null
      : transportProfit + (includeTickets ? ticketYourProfit : 0) + (includeTickets ? thaiDressYourProfit : 0)
    const partnerTotalProfit = (includeTickets ? ticketPartnerProfit : 0) + (includeTickets ? thaiDressPartnerProfit : 0)

    // 每人報價只除以成人（小孩不計入每人報價）
    const perPersonTHB = totalPrice === null ? null : totalPrice / adults
    const perPersonTWD = perPersonTHB === null ? null : Math.round(perPersonTHB / exchangeRate)

    return {
      people, adults, children, infants, carCount, carDistribution, maxPerCar, luggageStatus, luggageCheckCarCount, suggestLuggageCar, needLuggageCar, luggageCarCount: safeLuggageCarCount, nights, mealDays, guideDays, carServiceDays, childSeatDays, mealLevel,
      guided, vehicleType, perPerson,
      ...
      carCostTotal, carPriceTotal, legacyCarPriceReference, guideCost, guidePrice, luggageCost, childSeatCost,
      photographerCount,
  ...
  }, [config, adults, children, infants, people, exchangeRate, hotels, totalNights, mealLevel, mealServiceDays, tickets, thaiDressCloth, thaiDressPhoto, extraPhotographer, makeupCount, totalChildSeatCount, childSeatServiceDays, includeAccommodation, includeMeals, includeTickets, includeInsurance, includeGuide, carFees, outboundStayEnabled, outboundStayPerNight, outboundStayNights, outboundStayRooms])

  const fmt = (n: number | null) => n === null ? '待人工確認' : n.toLocaleString()
  const fmtRounded = (n: number | null) => n === null ? '待人工確認' : fmt(Math.round(n))
  ...
  const externalQuote = useMemo(
    () => {
      if (
        calculation.perPerson.manualQuoteRequired ||
        calculation.carPriceTotal === null ||
        calculation.totalPrice === null
      ) {
        return null
      }
      ...
      const breakdown = buildExternalQuoteBreakdown({
        pricingModel: 'perPerson',
        perPersonItems: calculation.perPerson.trip?.items ?? [],
        transferFee: calculation.perPerson.transferFee,
        transferTrips: calculation.perPerson.transferTrips,
        includeAccommodation,
        includeMeals,
        includeGuide: calculation.guided,
        includeInsurance,
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <button
          ...
          disabled={quoteGate.blocked}
          style={tabButtonStyle(activeTab === 'external', quoteGate.blocked ? '#999' : undefined)}
        >📄 對外報價單</button>
        <button onClick={() => {
          if (quoteGate.blocked) {
            alert(quoteGate.message)
        ...
        }} disabled={quoteGate.blocked} style={tabButtonStyle(false, quoteGate.blocked ? '#999' : '#b89b4d')}>📥 下載報價</button>
        <button
          onClick={() => {
            if (quoteGate.blocked) {
              alert(quoteGate.message)
          ...
          disabled={isGeneratingLink || quoteGate.blocked}
          style={tabButtonStyle(false, isGeneratingLink || quoteGate.blocked ? '#999' : '#2e7d32')}
        >
      ...
      {quoteGate.blocked && (
        <div style={{ background: '#ffebee', border: '2px solid #c62828', borderRadius: 8, padding: 12, marginBottom: 16 }}>
          <strong style={{ color: '#c62828' }}>⛔ 此單目前不能產生對客報價</strong>
          <div style={{ fontSize: 13, marginTop: 4 }}>{quoteGate.message}</div>
        </div>
      )}

      {/* 報價連結顯示 */}
      {!quoteGate.blocked && generatedUrl && (
        <div style={{
          display: 'flex',
          alignItems: 'center',
          gap: 8,
...
・機場送機`}
          ...
          <Section title="👥 基本設定">
            <div style={{ display: 'flex', gap: 24, alignItems: 'center', flexWrap: 'wrap', marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <input type="number" value={adults} onChange={e => setAdults(Math.max(1, Math.floor(Number(e.target.value) || 1)))} min={1} style={{ ...inputStyle, width: 60 }} />
                <span style={noteStyle}>人</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <label style={{ fontWeight: 'bold' }}>小孩</label>
                <input type="number" value={children} onChange={e => setChildren(Math.max(0, Math.floor(Number(e.target.value) || 0)))} min={0} style={{ ...inputStyle, width: 60 }} />
                <span style={noteStyle}>人</span>
                <span style={{ ...noteStyle, color: '#888' }}>（3-11歲，團費8折）</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <label style={{ fontWeight: 'bold' }}>嬰兒</label>
                <input type="number" value={infants} onChange={e => setInfants(Math.max(0, Math.floor(Number(e.target.value) || 0)))} min={0} style={{ ...inputStyle, width: 60 }} />
                <span style={noteStyle}>人</span>
                <span style={{ ...noteStyle, color: '#888' }}>（0-2歲，團費半價，佔位計級距）</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <label style={{ fontWeight: 'bold' }}>匯率</label>
                <input type="number" value={exchangeRate} onChange={e => setExchangeRate(Number(e.target.value))} min={0.9} max={1.3} step={0.01} style={{ ...inputStyle, width: 80 }} />
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <label style={{ fontWeight: 'bold' }}>旅客描述</label>
                <input type="text" value={travelerLabel} onChange={e => setTravelerLabel(e.target.value)} placeholder={children + infants > 0 ? `${adults}大${children > 0 ? `${children}小` : ''}${infants > 0 ? `${infants}嬰` : ''}` : `${people} 位貴賓`} style={{ ...inputStyle, width: 140 }} />
            ...
            >
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: guideControlPolicy.disabled ? 'not-allowed' : 'pointer' }}>
                <input
                  type="checkbox"
                  checked={includeGuide}
                  disabled={guideControlPolicy.disabled}
                  onChange={e => setIncludeGuide(e.target.checked)}
                  style={{ width: 16, height: 16 }}
                />
                <span>
                  🧑‍💼 含導遊
                  {guideControlPolicy.note ? <span style={{ color: '#9a6b2a', fontSize: 12 }}>{guideControlPolicy.note}</span> : null}
                </span>
              </label>
            ...
            {(includeMeals || calculation.guided) && (
              <div
                style={{
                  marginTop: 12,
                  padding: 10,
                  ...
                  {calculation.guided && (
                    ...
                    >
                      <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 'bold' }}>
                        ...
                        >
                          <option value={calculation.guideDays}>{calculation.guideDays} 天</option>
                        </select>
                      </label>
                    </div>
                  )}
            ...
            {(!includeAccommodation || !includeMeals || noActivitiesSelected || !includeInsurance || !calculation.guided) && (
          ...
          <Section title="🚗 車導費">
            ...
            {calculation.luggageStatus === 'ok' ? (
              ...
              <div style={{ background: '#ffebee', padding: 10, borderRadius: 6, marginBottom: 12 }}>
                <div style={{ fontSize: 13, color: '#c62828', background: '#fff', padding: 8, borderRadius: 4 }}>
                  🧳 配車後有單台 Van 分到 8 位客人，接機或送機每趟固定安排 1 台行李車。<br />
                  8–9 人及 15–18 人會加；10–14 人分乘兩台後不加。載客 Van 仍為 700／台／趟；額外行李車按實收 500／台／趟。
                </div>
              </div>
            )}
                  ...
                  <div key={index}>
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        ...
                        />
                        <select
                          value={['city', 'suburban', 'chiangrai', 'goldentriangle', 'airport'].includes(cf.type) ? cf.type : 'suburban'}
                          onChange={e => updateCarFee(index, 'type', e.target.value)}
                          style={{ padding: 4, border: '1px solid #ddd', borderRadius: 4, fontSize: 11, color: '#5c4a2a' }}
                        >
                          <option value="city">T1 市區</option>
                          <option value="suburban">T2 近郊</option>
                          <option value="chiangrai">T3 清萊</option>
                          <option value="goldentriangle">T4 金三角</option>
                          <option value="airport">純接送（按車收）</option>
                        </select>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        <span style={{ fontSize: 10, color: '#888' }}>黃表價(參考)</span>
                        <input
                          type="number"
                          value={cf.price}
            ...
            <div style={{ marginTop: 12, padding: 12, background: '#f9f8f6', borderRadius: 8 }}>
              ...
              <p style={{ margin: '4px 0 0 0', fontSize: 12, color: '#555' }}>
                人頭團費 {fmt(calculation.carPriceTotal)}（車＋導遊{calculation.needLuggageCar ? '＋機場行李車' : ''}攤入）
                {calculation.perPerson.transferFee > 0 ? `，含接送機 ${fmt(calculation.perPerson.transferFee)}` : ''}
        ...
        <Section title="📊 成本/售價/利潤明細（內部用）">
          ...
          <table style={{ width: '100%', minWidth: responsive.internalTableMinWidth, borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              ...
              <SectionRow title={`🚗 每日車費（黃表內部成本參考，${calculation.carCount}台）`} />
              ...
              <SubtotalRow name="黃表小計（僅參考，非售價）" cost={calculation.carCostTotal} price={calculation.legacyCarPriceReference} profit={calculation.legacyCarPriceReference - calculation.carCostTotal} />

              <SectionRow title="🧮 人頭團費（實際售價）" />
              {(calculation.perPerson.trip?.items ?? []).map((item, i) => (
                <DataRow key={`pp-${i}`} name={`${item.label} (${item.quantity} × ${fmt(item.unitPriceThb)})`} cost={0} price={item.subtotalThb} profit={0} className="day-row" />
              ))}
              {calculation.perPerson.transferFee > 0 && <DataRow name={`接送機（${calculation.perPerson.transferTrips}趟，按車收）`} cost={0} price={calculation.perPerson.transferFee} profit={0} className="day-row" />}
              <DataRow name="導遊實際成本（引擎依車數與非純接送行程日計算；售價已攤入團費）" cost={calculation.guideCost} price={0} profit={0} className="day-row" />
              <SubtotalRow
                name="團費小計"
                cost={calculation.transportCost}
                price={calculation.carPriceTotal}
                profit={calculation.carPriceTotal === null || calculation.transportCost === null
                  ? null
                  : calculation.carPriceTotal - calculation.transportCost}
              />
              ...
              <InfoRow text="※ 純接送日按車收（轎車500／Van 700／台／趟）；行李車依分車後人數：6–7 不加、8–9 加一台500、10–14 不加、15–18 加一台500" />
          ...
          <table style={{ width: '100%', minWidth: responsive.internalTableMinWidth, borderCollapse: 'collapse', fontSize: 13, marginTop: 16 }}>
            <tbody>
              ...
              <tr style={{ background: '#f9f8f6', fontWeight: 'bold' }}>
                ...
                <td style={tdStyle}>{fmt(calculation.totalCost)}</td>
                <td style={tdStyle}>{fmt(calculation.totalPrice)}</td>
                <td style={tdStyle}></td>
              </tr>
              ...
              {variant === 'formal' ? (
                ...
                <>
                  ...
                  <tr style={{ background: '#fff3cd' }}>
                    ...
                    <td style={{ ...tdStyle, fontWeight: 'bold' }}>{fmt(calculation.transportCost === null
                      ? null
                      : calculation.transportCost + calculation.ticketCost + calculation.mealCost + calculation.thaiDressCost)}</td>
                    <td style={tdStyle}></td>
                    <td style={tdStyle}></td>
                  </tr>
                </>
              ...
              <tr style={{ background: '#f9f8f6', fontWeight: 'bold' }}>
                ...
                <td style={tdStyle}></td>
                <td style={tdStyle}>{fmtRounded(calculation.perPersonTHB)} 泰銖</td>
      ...
      {/* External Tab */}
      {activeTab === 'external' && quoteGate.blocked && (
        <div style={{ background: '#ffebee', border: '2px solid #c62828', borderRadius: 8, padding: 12, marginBottom: 12 }}>
          <strong style={{ color: '#c62828' }}>⛔ 對客報價已阻擋</strong>
          <div style={{ fontSize: 13, marginTop: 4 }}>{quoteGate.message}</div>
        </div>
      )}
      {activeTab === 'external' && !quoteGate.blocked && externalQuote && (
        <ExternalQuoteTab
        ...
        <div style={{ background: 'white', border: '2px solid #5c4a2a', borderRadius: 12, padding: responsive.isCompact ? 16 : 24, maxWidth: 600, margin: '0 auto', width: '100%', boxSizing: 'border-box' }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ background: '#fafafa', borderRadius: 8, padding: 16 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '2px solid #5c4a2a', marginBottom: 8, marginTop: 12 }}>
                <span style={{ fontWeight: 'bold', color: '#5c4a2a' }}>🚗 團費（{calculation.carCount}台車{calculation.guided ? '，含中文導遊' : ''}）</span>
              ...
              <div style={{ paddingLeft: 16, fontSize: 12, color: '#555', lineHeight: 1.8 }}>
                {(calculation.perPerson.trip?.items ?? []).map((item, i) => (
                  <span key={`ppd-${i}`}>{i > 0 && <br />}• {item.label} {item.quantity} 位 × {fmt(item.unitPriceThb)}</span>
                ))}
                {calculation.perPerson.transferFee > 0 && <><br />• 接送機 {calculation.perPerson.transferTrips} 趟（按車計）</>}
          ...
          <div style={{ background: 'linear-gradient(135deg, #a08060 0%, #8b7355 100%)', color: 'white', padding: 20, borderRadius: 12, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 12, opacity: 0.8 }}>
              約 {fmtRounded(calculation.perPersonTHB)} 泰銖 ÷ {adults}成人
          ...
          {(() => {
            ...
            if (typeof carAmount !== 'number') return null
              ...
              <div style={{ marginTop: 20, background: '#f8f6f2', border: '1px solid #e8e4dc', borderRadius: 8, padding: 16 }}>
                ...
                {isCarOnly ? (
                  <>
                    ...
                    <div style={{ background: 'white', borderRadius: 6, padding: 12, marginBottom: 8, borderLeft: '4px solid #5c4a2a' }}>
                      ...
                      <div style={{ fontWeight: 'bold', color: '#5c4a2a', marginTop: 6 }}>
                        💰 {fmt(roundScaled(carAmount, 0.3))} 泰銖
                        <span style={{ fontWeight: 'normal', color: '#666', marginLeft: 8 }}>≈ NT$ {fmt(roundScaled(carAmount, 0.3 / exchangeRate))}</span>
                      </div>
                    </div>
                    <div style={{ background: 'white', borderRadius: 6, padding: 12, marginBottom: 8, borderLeft: '4px solid #5c4a2a' }}>
                      ...
                      <div style={{ fontSize: 12, color: '#555', lineHeight: 1.6 }}>
                        • 團費（含車{calculation.guided ? '＋導遊' : ''}{calculation.needLuggageCar ? '＋機場行李車' : ''}）{calculation.childSeatCost > 0 ? '、兒童座椅' : ''}<br />
                        • 統一結算超時費（若有）
                      </div>
                      <div style={{ fontWeight: 'bold', color: '#5c4a2a', marginTop: 6 }}>
                        💰 {fmt(roundScaled(carAmount, 0.7))} 泰銖
                        <span style={{ fontWeight: 'normal', color: '#666', marginLeft: 8 }}>≈ NT$ {fmt(roundScaled(carAmount, 0.7 / exchangeRate))}</span>
                      </div>
                    </div>
                  </>
                ) : includeAccommodation ? (
                  <>
                    ...
                    <div style={{ background: 'white', borderRadius: 6, padding: 12, marginBottom: 8, borderLeft: '4px solid #5c4a2a' }}>
                      <div style={{ fontWeight: 'bold', color: '#5c4a2a', marginBottom: 4 }}>📍 {hasMealsOrTicketsOrInsurance ? '第三' : '第二'}階段｜車{calculation.guided ? '導' : '輛'}費（含超時結算）</div>
                      <div style={{ fontSize: 12, color: '#666', marginBottom: 4 }}>⏰ 送機前一天</div>
                      <div style={{ fontSize: 12, color: '#555', lineHeight: 1.6 }}>
                        • 團費（含車{calculation.guided ? '＋導遊' : ''}{calculation.needLuggageCar ? '＋機場行李車' : ''}）{calculation.childSeatCost > 0 ? '、兒童座椅' : ''}<br />
                        • 統一結算超時費（若有）
                      ...
                      <div style={{ fontWeight: 'bold', color: '#5c4a2a', marginTop: 6 }}>
                        💰 {fmt(carAmount)} 泰銖
                        <span style={{ fontWeight: 'normal', color: '#666', marginLeft: 8 }}>≈ NT$ {fmt(roundScaled(carAmount, 1 / exchangeRate))}</span>
                      </div>
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div style={{ background: 'white', borderRadius: 6, padding: 12, marginBottom: 8, borderLeft: '4px solid #5c4a2a' }}>
                      <div style={{ fontWeight: 'bold', color: '#5c4a2a', marginBottom: 4 }}>📍 第二階段｜車{calculation.guided ? '導' : '輛'}訂金 30%</div>
                    ...
                    <div style={{ background: 'white', borderRadius: 6, padding: 12, marginBottom: 8, borderLeft: '4px solid #5c4a2a' }}>
                      <div style={{ fontWeight: 'bold', color: '#5c4a2a', marginBottom: 4 }}>📍 第三階段｜車{calculation.guided ? '導' : '輛'}尾款 70%（含超時結算）</div>
                      <div style={{ fontSize: 12, color: '#666', marginBottom: 4 }}>⏰ 送機前一天</div>
                      <div style={{ fontSize: 12, color: '#555', lineHeight: 1.6 }}>
                        • 團費（含車{calculation.guided ? '＋導遊' : ''}{calculation.needLuggageCar ? '＋機場行李車' : ''}）{calculation.childSeatCost > 0 ? '、兒童座椅' : ''}<br />
                        • 統一結算超時費（若有）
          ...
          {(() => {
            ...
            const carAmount = calculation.transportPrice
            const quotedPerPersonTWD = calculation.perPersonTWD
            if (typeof carAmount !== 'number' || typeof quotedPerPersonTWD !== 'number') return null
            const hasMeals = includeMeals && mealsAmount > 0
              ...
              <div style={{ marginTop: 20, background: '#5c4a2a', color: 'white', padding: 16, borderRadius: 8 }}>
                ...
                <div style={{ fontSize: 13, lineHeight: 2 }}>
                  {isCarOnly ? (
                    <>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <span>NT$ {fmt(roundScaled(carAmount, 0.3 / exchangeRate))}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>第二階段（尾款 70%）</span>
                        <span>NT$ {fmt(roundScaled(carAmount, 0.7 / exchangeRate))}</span>
                      </div>
                    </>
                  ) : includeAccommodation ? (
                    <>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>車{calculation.guided ? '導' : '輛'}費</span>
                        <span>NT$ {fmt(roundScaled(carAmount, 1 / exchangeRate))}</span>
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>車{calculation.guided ? '導' : '輛'}訂金 30%</span>
                        <span>NT$ {fmt(roundScaled(carAmount, 0.3 / exchangeRate))}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>車{calculation.guided ? '導' : '輛'}尾款 70%</span>
                        <span>NT$ {fmt(roundScaled(carAmount, 0.7 / exchangeRate))}</span>
                      </div>
                    </>
                  )}
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.3)', marginTop: 8, paddingTop: 8, display: 'flex', justifyContent: 'space-between', fontWeight: 'bold', fontSize: 15 }}>
                    <span>團費總計</span>
                    <span>NT$ {fmt(roundScaled(quotedPerPersonTWD, people))}</span>
                  </div>
...
function DataRow({ name, cost, price, profit, className }: { name: string; cost: number | null; price: number | null; profit: number | null; className?: string }) {
  return (
    <tr style={className === 'day-row' ? { background: '#fafafa' } : {}}>
      <td style={{ ...tdStyle, textAlign: 'left' }}>{name}</td>
      <td style={tdStyle}>{cost === null ? '待人工確認' : cost.toLocaleString()}</td>
      <td style={tdStyle}>{price === null ? '待人工確認' : price.toLocaleString()}</td>
      <td style={tdStyle}>{profit === null ? '待人工確認' : profit > 0 ? profit.toLocaleString() : '-'}</td>
    </tr>
  )
}

function SubtotalRow({ name, cost, price, profit }: { name: string; cost: number | null; price: number | null; profit: number | null }) {
  return (
    <tr style={{ background: '#f9f8f6', fontWeight: 'bold' }}>
      <td style={{ ...tdStyle, textAlign: 'left' }}>{name}</td>
      <td style={tdStyle}>{cost === null ? '待人工確認' : cost.toLocaleString()}</td>
      <td style={tdStyle}>{price === null ? '待人工確認' : price.toLocaleString()}</td>
      <td style={tdStyle}>{profit === null ? '待人工確認' : profit.toLocaleString()}</td>
    </tr>
  )
}
