...
const RESIDUAL_PII_RES: RegExp[] = [
  ...
  /[\w.+-]+@[\w.-]+\.\w+/u, // email
  /\d{4}[/／-]\d{1,2}[/／-]\d{1,2}/u, // slash/dash 日期
  /\d{4}\s?年\s?\d{1,2}\s?月\s?\d{1,2}\s?日/u, // 中文式年月日日期（scrub 不抓 → 補漏）
  // 中文數字金額：中文數字串緊接幣別單位（「五千銖／兩萬元」scrub 只抓阿拉伯數字會漏）。
  // 需緊接幣別單位才 trip → 「千人火鍋」（千+人）不誤殺。
  /[一二三四五六七八九十兩零百千萬]+\s?(?:泰銖|銖|元|塊)/u,
  // 敬稱姓氏：刻意做粗略子字串比對（會誤殺「太太樂／一家親／小姐姐」等合法字串）。
  // 這在寧缺勿漏下可接受：誤丟一筆只是退回 curated template，不洩漏。
  // 絕不可放寬成「需錨定前一個 CJK 姓氏字」——那會重新引入真實 leak 風險。
  /(?:先生|小姐|太太|一家)/u, // 敬稱姓氏
]
