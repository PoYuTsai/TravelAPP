...
/**
 * GOLDEN_LI_FAMILY_ELDERLY_7D6N — 清邁長輩友善 7 天 6 夜 golden 標準範本（第3刀）。
 *
 * 來源：docs/ai-agent-knowledge/cases/eric-four-golden-cases.md 案 1（李先生一家）。
 * 內容逐字取自 Eric 定稿之真實 ground truth（保留原換行、航班碼、`**` 強調、具名餐廳等內文），
 * **正規化**：標題去敬稱/真名改為 `<清邁長輩友善7天6夜>`；header 改通用占位；移除每日日曆日期前綴行
 * （如「8/4 (二)」），只留 Day N 區塊。其餘日程內文逐字保留。
 *
 * 注意：這是「權威範本」（保留具體航班/markdown），**絕不過 sanitizer**（同 5D4N/6D5N）。
 * Day 1..Day 7 連續標題與「日期/人數」占位為下游 hard dependency。
 */
export const GOLDEN_LI_FAMILY_ELDERLY_7D6N = `<清邁長輩友善7天6夜>
📅 日期：2026//～/
👨‍👩‍👧‍👦 人數：幾大幾小（幾歲，身高，需不需要兒童座椅，有無長輩，有無特殊備註事項）
...
Day 7｜收心慢遊・送機
・早餐後退房／飯店整理行李 (早上9:30送機)
平安返家`

/**
 * GOLDEN_NEWYEAR_5D4N — 清邁跨年 5 天 4 夜 golden 標準範本（第3刀）。
 *
 * 來源：docs/ai-agent-knowledge/cases/eric-four-golden-cases.md 案 4（跨年特殊案／CAD 天燈）。
 * 內容逐字取自 Eric 定稿之真實 ground truth（保留原換行、航班碼 CI851/CI852、`**` 強調、
 * 具名餐廳/飯店等內文），**正規化**：標題改為 `<清邁跨年5天4夜>`（原案 4 標題為 `<清邁5天4夜>`，
 * 為與經典 5D4N 區隔且消歧用，改通用描述）；header 改通用占位；移除每日日曆日期前綴行
 * （如「12/31 (四)」），只留 Day N 區塊。其餘日程內文逐字保留（含 CAD 跨年天燈、官網預訂備註）。
 *
 * 注意：這是「權威範本」（保留具體航班/markdown），**絕不過 sanitizer**（同其餘 golden）。
 * Day 1..Day 5 連續標題與「日期/人數」占位為下游 hard dependency。
 */
export const GOLDEN_NEWYEAR_5D4N = `<清邁跨年5天4夜>
📅 日期：2026//～/
👨‍👩‍👧‍👦 人數：幾大幾小（幾歲，身高，需不需要兒童座椅，有無長輩，有無特殊備註事項）

Day 1｜抵達清邁・放鬆展開旅程 + CAD跨年天燈祭典
・機場接機 (華航 CI851 7:20~10:25)
・Nakhonping Exchange換匯
午餐：Neng's Clay Oven Roasted Pork – Muang Mai Market (清邁必吃脆皮豬)
・回飯店check in + 休息
・準備去CAD現場 (提早出發怕塞車)
晚餐: CAD會場提供天燈+水燈
・CAD跨年天燈秀 (票要提前官網預定)
・住宿：Wintree City Resort Chiang Mai
...
Day 3｜市集 + 清邁藍廟 + 天使瀑布 + 夜間動物園
**8:00出發**
・真心市集
・清邁藍廟
・天使瀑布
午餐: The Kad Farang Mae Rim (很多餐廳可選)
・藝術村 (Baan Kangwat，禮拜二沒開)
・夜間動物園
晚餐：黑森林餐廳 (禮拜三沒開)
・住宿：Wintree City Resort Chiang Mai
...
早餐後退房
**9:30送機**
・安排送機服務 (華航 CI852  11:25~15:50) ，結束愉快又充實的泰北親子旅程`
