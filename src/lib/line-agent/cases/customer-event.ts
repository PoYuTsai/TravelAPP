...
/** Product / itinerary content questions (non-price). */
const PRODUCT_PATTERN = /哪間|哪一間|哪一家|含不含|包含|包不包|有沒有含|幾點|行程|景點|怎麼安排|可以去|能去|去哪/

/** Generic question markers. */
const QUESTION_PATTERN = /嗎|吗|\?|？/
...
/** Small-talk / greeting / sticker words → non_actionable (NOT a catch-all). */
...
export function classifyCustomerEventDeterministic(
  ...
  // 5. product / itinerary question (non-price, needs a question marker).
  if (PRODUCT_PATTERN.test(trimmed) && QUESTION_PATTERN.test(trimmed)) {
  ...
  // 8. small talk / greeting → non_actionable.
  if (SMALLTALK_PATTERN.test(trimmed)) {
